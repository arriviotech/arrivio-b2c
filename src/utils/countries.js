// Full ISO 3166 country list with international dialing codes.
// Used by the application form for the Country dropdown (AddressDetails)
// and the phone-number country-code selector (PersonalDetails).
//
// NOTE: Names for Germany, Austria, Switzerland, Netherlands, France and
// United Kingdom must stay exactly as below — AddressDetails matches them
// against its CITIES_BY_COUNTRY map.
...
// Derive the flag emoji from an ISO2 code (regional indicator symbols).
// On platforms without emoji-flag support (e.g. Windows) this renders as
// the two-letter code, e.g. "DE" — which is still a useful label.
const flagFromIso = (iso2) =>
  iso2
    .toUpperCase()
    .replace(/./g, (ch) => String.fromCodePoint(127397 + ch.charCodeAt(0)));

// Sorted alphabetically by name, with flag attached.
export const COUNTRIES = RAW.map((c) => ({ ...c, flag: flagFromIso(c.iso2) })).sort(
  (a, b) => a.name.localeCompare(b.name)
);

// Just the country names (for the address Country dropdown).
export const COUNTRY_NAMES = COUNTRIES.map((c) => c.name);

// Dial codes longest-first — used to reliably strip a country code from a
// stored phone number (so "+971…" matches before "+9…", "+1242…" before "+1").
export const DIAL_CODES_LONGEST_FIRST = [
